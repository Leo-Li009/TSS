...
async fn getinfer(db: &State<Db>, room_id: &str,last_seen_msg: LastEventId,) -> Json<Vec<String>>{
    let room = db.get_room_or_create_empty(room_id).await;
...
async fn broadcastC(db: &State<Db>, room_id: &str, message: String) -> Status {
    
    let room = db.get_room_or_create_empty(room_id).await;
    println!("room-id{}",room_id);
    //let mut subs=room.clone().subscribe(last_seen_msg.0);
    let t = room.subscribers.fetch_add(1, Ordering::SeqCst);
    println!("broadcasting-c message: {}", message);
    room.publish(message).await;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    rocket::custom(figment)
        .mount("/", rocket::routes![subscribe, issue_idx, broadcast,broadcastC,getinfer])
        .manage(Db::empty())
        .launch()
        .await?;
    Ok(())
